...
} from '../swagger/decorators/residential-office/get-properties.decorator';
...
export class ResidentialOfficeController {
  ...
  async createPurchase(@Req() req, @Body() dto: CreatePurchaseDto) {
    ...
    return successResponse(
      [],
      'تم إنشاء عملية الشراء وإصدار الفواتير بنجاح',
      HttpStatus.CREATED,
    );
  }
  ...
  async getProperties(@CurrentUser() user, @Req() request: Request) {
    const userId = user.sub;

    const baseUrl = `${request.protocol}://${request.get('host')}`;

    const properties = await this.getPropertiesForOfficeUseCase.execute(
      userId,
      baseUrl,
    );
    ...
    return successResponse(
      properties,
      'تم ارجاع جميع العقارات الخاصة بمكتبك',
      200,
    );
  }
  ...
  async searchProertiesWithFilters(
    ...
    const properties =
      await this.getPropertiesForOfficeWithFiltersUseCase.execute(
        userId,
        filters,
        baseUrl,
      );
    ...
    return successResponse(
      properties,
      'تم ارجاع جميع العقارات الخاصة بمكتبك مفلترة',
      200,
    );
  }
  ...
  async seacrhPropertiesByTitle(
    ...
    const properties =
      await this.searchResidentialPropertyByTitleUseCase.execute(
        userId,
        search.title,
        baseUrl,
      );
    ...
    return successResponse(
      properties,
      'تم ارجاع جميع العقارات الخاصة المتعقلة بالبحث',
      200,
    );
  }
  ...
  async getProperty(
    ...
    const property = await this.getPropertyForOfficeUseCase.execute(
      userId,
      propertyId,
      baseUrl,
    );
  ...
  async addResidentialProperty(
    ...
    if (!file || !file.filename) {
      throw new BadRequestException(errorResponse('يجب رفع صورة للإعلان', 400));
    }

    const imageName = file?.filename;

    const propertyId =
      await this.createResidentialPropertyDetailsUseCase.execute(
        residentialDetails,
        userId,
        imageName,
      );
  ...
  @PropertyPostImageInterceptor()
  async updateResidentialProeprty(
    @Param('propertyId') propertyId: number,
    @UploadedFile() file: Express.Multer.File,
