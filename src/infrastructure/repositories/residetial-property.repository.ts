  ...
  implements ResidentialPropertyRepositoryInterface
{
  ...
  async updateStatusOfProperty(propertyId: number, status: PropertyStatus) {
    const residentialProperty = await this.residentialRepo.findOne({
  ...
  async findOneByPropertyId(propertyId: number): Promise<Residential | null> {
    ...
    return await this.residentialRepo
  ...
  async updateResidentialProperty(
    propertyId: number,
    data: UpdateResidentialPropertyDetailsDto,
  ) {
    const residentialProperty = await this.residentialRepo.findOne({
      where: { property: { id: propertyId } },
    });
  ...
  async searchFilteredResidentialsProperties(
    baseUrl: string,
    filters: ResidentialPropertiesSearchFiltersDto,
    page: number,
    items: number,
    userId: number,
  ) {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (
        value !== undefined &&
        value !== null &&
        filterMap.has(key as keyof ResidentialPropertiesSearchFiltersDto)
      ) {
        filterMap.get(key as keyof ResidentialPropertiesSearchFiltersDto)!(
          query,
          value,
        );
      }
    ...
    const final = entities.map((entity, index) => ({
      ...
      property: {
        ...
        is_favorite:
          raw[index]?.is_favorite === true || raw[index]?.is_favorite === 'true'
            ? 1
            : 0,
      },
    }));

    const formatted = final.map((residential) =>
      this.formatResidential(residential, baseUrl),
    );
  ...
  private formatResidential(residential, baseUrl: string) {
    ...
    const base = {
      ...
      postImage: `${baseUrl}/uploads/properties/posts/images/${
        property.post?.image ?? 'default.jpg'
      }`,
      location: `${property.region?.city?.name ?? ''}, ${
  ...
  private getFilterMap() {
    return new Map<
      keyof ResidentialPropertiesSearchFiltersDto,
      (query: any, value: any) => void
    >([
      [
        'regionId',
        (q, v) => q.andWhere('property.region_id = :regionId', { regionId: v }),
      ],
      ['cityId', (q, v) => q.andWhere('city.id = :cityId', { cityId: v })],
      [
        'listing_type',
        (q, v) =>
          q.andWhere('residential.listing_type = :listing_type', {
            listing_type: v,
          }),
      ],
      [
        'ownership_type',
        (q, v) =>
          q.andWhere('residential.ownership_type = :ownership_type', {
            ownership_type: v,
          }),
      ],
      [
        'status',
        (q, v) => q.andWhere('residential.status = :status', { status: v }),
      ],
      [
        'has_furniture',
        (q, v) =>
          q.andWhere('property.has_furniture = :has_furniture', {
            has_furniture: v,
          }),
      ],
      [
        'direction',
        (q, v) =>
          q.andWhere('residential.direction = :direction', { direction: v }),
      ],
      [
        'minPrice',
        (q, v) =>
          q.andWhere(
            '(residential.rental_price >= :minPrice OR residential.selling_price >= :minPrice)',
            { minPrice: v },
          ),
      ],
      [
        ...
        (q, v) =>
          q.andWhere(
            '(residential.rental_price <= :maxPrice OR residential.selling_price <= :maxPrice)',
            { maxPrice: v },
          ),
      ],
      [
        'minArea',
        (q, v) => q.andWhere('property.area >= :minArea', { minArea: v }),
      ],
      [
        'maxArea',
        (q, v) => q.andWhere('property.area <= :maxArea', { maxArea: v }),
      ],
      [
        'floor_number',
        (q, v) =>
          q.andWhere('property.floor_number = :floor_number', {
            floor_number: v,
          }),
      ],
    ]);
  ...
  private applyRoomDetailsFilter(
    query: any,
    roomDetails?: ResidentialPropertiesSearchFiltersDto['room_details'],
  ) {
  ...
  private buildBaseQuery() {
    return this.residentialRepo
      ...
      .andWhere('post.status = :statusPost', {
        statusPost: PropertyPostStatus.APPROVED,
      })
  ...
  private buildUpdatePayload(
    ...
    if (data.ownership_type !== undefined)
      payload.ownership_type = data.ownership_type;
    if (data.direction !== undefined) payload.direction = data.direction;

    // Rent details
    if (data.rent_details) {
      const rent = data.rent_details;
      if (rent.rental_period !== undefined)
        payload.rental_period = rent.rental_period;
      if (rent.rentalPrice !== undefined)
        payload.rental_price = rent.rentalPrice;
    }

    // Sell details
    if (data.sell_details) {
      const sell = data.sell_details;
      if (sell.selling_price !== undefined)
        payload.selling_price = sell.selling_price;
      if (sell.installment_allowed !== undefined)
        payload.installment_allowed = sell.installment_allowed;
      if (sell.installment_duration !== undefined)
        payload.installment_duration = sell.installment_duration;
    }

    return payload;
  }

  async findTopResidentialLocationsByOffice(
    officeId: number,
  ): Promise<string[]> {
  ...
  async createPurchaseWithInvoices(options: {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      if (!property.residential)
        throw new NotFoundException(
          'هذا العقار ليس سكنياً أو تفاصيله غير موجودة',
        );
      ...
      const firstDueDeadline =
        bookingPeriodDays > 0
          ? addDays(nowStart, bookingPeriodDays)
          : addDays(nowStart, 1);
      if (!installment) {
        ...
        if (!months || months <= 0) {
          throw new BadRequestException(
            'مدة التقسيط غير معرفة في تفاصيل العقار',
          );
        }
        ...
        for (let i = 0; i < months; i++) {
          ...
          const paymentDeadline =
            i === 0 ? firstDueDeadline : addMonths(dueDate, 1);
