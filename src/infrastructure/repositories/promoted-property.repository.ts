...
export class PromotedPropertyRepository
  implements PromotedPropertyRepositoryInterface
{
  constructor(
  ...
  async getActivePromotedProperties() {
    return this.promotedProeprtyRepo
      .createQueryBuilder('pp')
      .leftJoin('pp.onlineInvoice','onlineInvoice')
      ...
      .select([
        ...
        'onlineInvoice.amount AS amount',
        'pp.period AS period',
        'post.title AS title',
      ...
      .andWhere(
        `pp.start_date + (pp.period || ' days')::interval >= CURRENT_DATE`,
      )
      .getRawMany();
  }
  ...
  async getAllPromotedProperties(page: number, items: number, userId: number) {
    ...
    const baseQuery = this.propertyRepo
      ...
      .where('property.property_type = :type', {
      ...
      .andWhere(
        `(promoted.start_date + (promoted.period || ' days')::interval) >= CURRENT_DATE`,
      )
      .select([
