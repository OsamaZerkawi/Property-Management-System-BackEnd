...
export class UserPostRepository implements UserPostRepositoryInterface {
  ...
  async update(id: number, fields: Partial<UserPost>) {
    await this.userPostRepo.update(id, fields);
  }
  async getPostsByStatus(status: UserPostAdminAgreement) {
    const posts = await this.userPostRepo.find(
      this.buildFindOptions({ status }),
    );
  ...
  async findById(id: number) {
    return await this.userPostRepo.findOne({
      where: { id },
      relations: ['user'],
    });
  }
