...
export class GetOfficeDetailsUseCase {
 ...
 async execute(userId: number, baseUrl: string) {
  ...
  const platforms = await this.officeRepo.getAllSocialPlatformsWithOfficeLinks(office.id);
  ...
  return {
    ...
    creditCard:office.creditCard,
    opening_time: office.opening_time,
    closing_time: office.closing_time,
    latitude: office.latitude,
    longitude: office.longitude,
    region: office.region
      ? { id: office.region.id, name: office.region.name }
      : null,
    city: office.region?.city
      ? { id: office.region.city.id, name: office.region.city.name }
      : null,
