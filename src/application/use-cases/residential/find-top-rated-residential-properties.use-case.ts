...
export class FindTopRatedResidentialPropertiesUseCase {
    constructor(
    ...
    async execute(page: number,items: number,type: PropertyType,baseUrl: string,userId: number){
        const { raw,total} = await this.propertyRepo.getTopRatedResidentialProperties(page,items,type,userId);

        const results = raw.map(row => {
          const base = {
            id: row.property_id,
            title: row.post_title,
            image: `${baseUrl}/uploads/${row.post_image}`,
